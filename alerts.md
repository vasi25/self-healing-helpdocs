...
- **Subscribed Alert** — An Alert linked to a notification profile to which the current user is the owner or associate.

## Alert Notifications

Alert notifications is emailed to each subscribed user after an Audit run completes. You can change the frequency of notifications when you create an Alert — sent every time its triggered, or only once when newly triggered (resetting when the Alert is no longer triggered).
